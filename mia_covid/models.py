...
def resnet_block(x, filters, name, kernel_size=3, stride=1,
                 conv_shortcut=False):
...
def resnet_stack(x, filters, blocks, name, stride1=2):
...
class model_settings:
    architecture: str
    batch_size: int
    batch_size_private_covid: int
    # model_shape: Tuple[int, int, int] -> save the ideal shape for the model?


resnet18 = model_settings(architecture="resnet18", batch_size=32, batch_size_private_covid=16)
resnet50 = model_settings(architecture="resnet50", batch_size=32, batch_size_private_covid=8)
